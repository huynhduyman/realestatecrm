...
def update_lead(request, pk):
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if request.POST.get("saveandcreateopportunityform"):
                success_url = reverse("opportunities:create", kwargs={
                                      'view_account': 3})
            #success_url = reverse("opportunities:save", params={'view_account': lead_obj.id})
            path = reverse("opportunities:save")
            params = urlencode({'view_lead': lead_obj.id})
            success_url = "%s?%s" % (path, params)
