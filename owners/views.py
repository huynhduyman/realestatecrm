...
from owners.forms import (OwnerAttachmentForm, OwnerCommentForm,
                            OwnerForm, EmailForm)
from owners.models import Owner, Email, Tags
from owners.tasks import send_email, send_email_to_assigned_user
...
class OwnersListView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    model = Owner
    ...
    def get_context_data(self, **kwargs):
        context = super(OwnersListView, self).get_context_data(**kwargs)
        open_owners = self.get_queryset().filter(status='open')
        close_owners = self.get_queryset().filter(status='close')
        context["owners_list"] = self.get_queryset()
        context["users"] = User.objects.filter(
            is_active=True).order_by('email')
        context['open_owners'] = open_owners
        context['close_owners'] = close_owners
        context["industries"] = INDCHOICES
        context["per_page"] = self.request.POST.get('per_page')
        tag_ids = list(set(Owner.objects.values_list('tags', flat=True)))
...
class CreateOwnerView(SalesAccessRequiredMixin, LoginRequiredMixin, CreateView):
    model = Owner
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(
            CreateOwnerView, self).dispatch(request, *args, **kwargs)

    def get_form_kwargs(self):
        kwargs = super(CreateOwnerView, self).get_form_kwargs()
    ...
    def form_valid(self, form):
        # Save owner
        owner_object = form.save(commit=False)
        owner_object.created_by = self.request.user
        owner_object.save()
        ...
        if self.request.POST.get('tags', ''):
            ...
            for t in splitted_tags:
                ...
                owner_object.tags.add(tag)
        if self.request.POST.getlist('contacts', []):
            owner_object.contacts.add(*self.request.POST.getlist('contacts'))
        if self.request.POST.getlist('assigned_to', []):
            owner_object.assigned_to.add(*self.request.POST.getlist('assigned_to'))
        if self.request.FILES.get('owner_attachment'):
            attachment = Attachments()
            attachment.created_by = self.request.user
            attachment.file_name = self.request.FILES.get(
                'owner_attachment').name
            attachment.owner = owner_object
        ...
        if self.request.POST.getlist('teams', []):
            ...
            assinged_to_users_ids = owner_object.assigned_to.all().values_list('id', flat=True)
            for user_id in user_ids:
                if user_id not in assinged_to_users_ids:
                    owner_object.assigned_to.add(user_id)
        if self.request.POST.getlist('teams', []):
            owner_object.teams.add(*self.request.POST.getlist('teams'))

        assigned_to_list = list(owner_object.assigned_to.all().values_list('id', flat=True))
        current_site = get_current_site(self.request)
        recipients = assigned_to_list
        send_email_to_assigned_user.delay(recipients, owner_object.id, domain=current_site.domain,
            protocol=self.request.scheme)

        if self.request.POST.get("savenewform"):
            return redirect("owners:new_owner")

        if self.request.is_ajax():
            data = {'success_url': reverse_lazy(
                'owners:list'), 'error': False}
    ...
    def get_context_data(self, **kwargs):
        ...
        context["owner_form"] = context["form"]
...
class OwnerDetailView(SalesAccessRequiredMixin, LoginRequiredMixin, DetailView):
    model = Owner
    context_object_name = "owner_record"
    template_name = "view_owner.html"

    def get_context_data(self, **kwargs):
        context = super(OwnerDetailView, self).get_context_data(**kwargs)
        owner_record = context["owner_record"]
        if self.request.user.role != "ADMIN" and not self.request.user.is_superuser:
            if not ((self.request.user == owner_record.created_by) or
                (self.request.user in owner_record.assigned_to.all())):
                raise PermissionDenied

        comment_permission = True if (
            self.request.user == owner_record.created_by or
        ...
        elif self.request.user != owner_record.created_by:
            if owner_record.created_by:
                users_mention = [{'username': owner_record.created_by.username}]
            else:
                users_mention = []
        else:
            users_mention = []
        ...
        context.update({
            "comments": owner_record.owners_comments.all(),
            "attachments": owner_record.owner_attachment.all(),
            "opportunity_list": Opportunity.objects.filter(
                owner=owner_record),
            "users": User.objects.filter(is_active=True).order_by('email'),
            "cases": Case.objects.filter(owner=owner_record),
            "stages": STAGES,
            "sources": SOURCES,
            "countries": COUNTRIES,
            ...
            'tasks':owner_record.owners_tasks.all(),
            'invoices':owner_record.owners_invoices.all(),
            'emails':owner_record.sent_email.all(),
            'users_mention': users_mention,
        })
        return context


class OwnerUpdateView(SalesAccessRequiredMixin, LoginRequiredMixin, UpdateView):
    model = Owner
    form_class = OwnerForm
    template_name = "create_owner.html"
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # else:
        #     self.users = User.objects.filter(role='ADMIN').order_by('email')
        return super(OwnerUpdateView, self).dispatch(
            request, *args, **kwargs)
    ...
    def form_valid(self, form):
        ...
        owner_object = form.save(commit=False)
        owner_object.save()
        previous_assigned_to_users = list(owner_object.assigned_to.all().values_list('id', flat=True))
        owner_object.tags.clear()
        if self.request.POST.get('tags', ''):
            tags = self.request.POST.get("tags")
            splitted_tags = tags.split(",")
            for t in splitted_tags:
                ...
                owner_object.tags.add(tag)
        if self.request.POST.getlist('contacts', []):
            owner_object.contacts.clear()
            owner_object.contacts.add(*self.request.POST.getlist('contacts'))
        if self.request.POST.getlist('assigned_to', []):
            owner_object.assigned_to.clear()
            owner_object.assigned_to.add(*self.request.POST.getlist('assigned_to'))
        else:
            owner_object.assigned_to.clear()
        if self.request.FILES.get('owner_attachment'):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["owner_obj"] = self.object
        if self.request.user.role != "ADMIN" and not self.request.user.is_superuser:
            if ((self.request.user != context['owner_obj'].created_by ) and
                (self.request.user not in context['owner_obj'].assigned_to.all())):
                raise PermissionDenied
        context["owner_form"] = context["form"]
        if self.request.user.role != "ADMIN" and not self.request.user.is_superuser:
...
class OwnerDeleteView(SalesAccessRequiredMixin, LoginRequiredMixin, DeleteView):
    model = Owner
    template_name = 'view_owner.html'
...
class AddCommentView(LoginRequiredMixin, CreateView):
    model = Comment
    form_class = OwnerCommentForm
    http_method_names = ["post"]

    def post(self, request, *args, **kwargs):
        self.object = None
        self.owner = get_object_or_404(
            Owner, id=request.POST.get('ownerid'))
        if (
            request.user == self.owner.created_by or request.user.is_superuser or
            request.user.role == 'ADMIN' or request.user in self.owner.assigned_to.all()
        ):
            form = self.get_form()
            if form.is_valid():
                return self.form_valid(form)
        ...
        data = {
            'error': "You don't have permission to comment for this owner."}
        return JsonResponse(data)
    ...
    def form_valid(self, form):
        ...
        current_site = get_current_site(self.request)
        send_email_user_mentions.delay(comment_id, 'owners', domain=current_site.domain,
            protocol=self.request.scheme)
...
class UpdateCommentView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user == self.comment_obj.commented_by:
            form = OwnerCommentForm(request.POST, instance=self.comment_obj)
            if form.is_valid():
                return self.form_valid(form)
...
class AddAttachmentView(LoginRequiredMixin, CreateView):
    model = Attachments
    form_class = OwnerAttachmentForm
    http_method_names = ["post"]
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.owner = get_object_or_404(
            Owner, id=request.POST.get('ownerid'))
        if (
            request.user == self.owner.created_by or
            request.user.is_superuser or
            request.user.role == 'ADMIN' or
            request.user in self.owner.assigned_to.all()
        ):
            form = self.get_form()
            if form.is_valid():
                return self.form_valid(form)
...
def create_mail(request):
    if request.method == 'GET':
        owner = get_object_or_404(Owner, pk=request.GET.get('owner_id'))
        contacts_list = list(owner.contacts.all().values('email'))
        TIMEZONE_CHOICES = [(tz, tz) for tz in pytz.common_timezones]
        email_form = EmailForm(owner=owner)
        return render(request, 'create_mail_owners.html', {'owner_id': request.GET.get('owner_id'),
            'contacts_list': contacts_list, 'email_form': email_form,
            "timezones": TIMEZONE_CHOICES, "settings_timezone": settings.TIME_ZONE})

    if request.method == 'POST':
        owner = Owner.objects.filter(id=request.POST.get('owner_id')).first()
        if owner:
            form = EmailForm(request.POST, owner=owner)
            if form.is_valid():
            ...
            return JsonResponse({'error':True, 'message': "owner does not exist."})

# @login_required
# @sales_access_required
# def create_mail(request, pk):
#     owner_obj = get_object_or_404(Owner, pk=pk)
#     if request.method == 'GET':
#         context = {}
#         context['owner_obj'] = owner_obj
#         return render(request, 'create_email_for_contacts.html')


def get_contacts_for_owner(request):
    owner_id = request.POST.get('owner_id')
    if owner_id:
        owner_obj = Owner.objects.filter(pk=owner_id).first()
        if owner_obj:
            if owner_obj.contacts.all():
                data = list(owner_obj.contacts.values('id', 'email'))
                import json
...
def get_email_data_for_owner(request):
    email_obj = Email.objects.filter(id=request.POST.get('email_owner_id')).first()
    if email_obj:
        ctx  = {}
