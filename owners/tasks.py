...
from owners.models import Owner, Email, EmailLog
...
def send_email_to_assigned_user(recipients, from_email, domain='demo.django-crm.io', protocol='http'):
    ...
    owner = Owner.objects.filter(id=from_email).first()
    ...
    for user in recipients:
        ...
        if user:
            ...
            context["url"] = protocol + '://' + domain + \
                reverse('owners:view_owner', args=(owner.id,))
            context["user"] = user
            context["owner"] = owner
            context["created_by"] = created_by
            subject = 'Assigned a owner for you.'
            html_content = render_to_string(
                'assigned_to/owner_assigned.html', context=context)
