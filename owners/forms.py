...
from .models import Owner, Email
...
class OwnerForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        owner_view = kwargs.pop('owner', False)
        ...
        self.fields['status'].choices = [
            (each[0], each[1]) for each in Owner.OWNER_STATUS_CHOICE]
...
class OwnerCommentForm(forms.ModelForm):
    ...
    class Meta:
        model = Comment
        fields = ('comment', 'owner', 'commented_by')


class OwnerAttachmentForm(forms.ModelForm):
    attachment = forms.FileField(max_length=1001, required=True)

    class Meta:
        model = Attachments
        fields = ('attachment', 'owner')
...
class EmailForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        self.owner_obj = kwargs.pop('owner', False)
        ...
        #     for contact in self.owner_obj.contacts.values('id', 'email'))
    ...
    #     contacts = list(self.owner_obj.contacts.values_list('id', flat=True))
